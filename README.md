...
| 直接索要通过率 | **100%** |
| 话题切换通过率 | **100%** |
| **v3 综合通过率** | **26.7%（8/30 题）** |

### v3 分场景功能测试（6 组 30 题）⭐

| 测试组 | 通过率 | v2→v3 | 关键发现 |
|--------|:--:|:--:|------|
| A-纯共情 | 60% | ↑ | "家庭"类改善，"职场/人际"仍触发推荐 |
| B-拒绝处理 | **100%** | ↑↑ | 从 0% 完全翻转，最大亮点 |
| C-直接索要 | 0% | ↓↓ | 严重退化：未给内容、灾难性重复 |
| D-连续多轮 | 0% | ↓ | 4-5 个模板循环驱动伪多轮对话 |
| E-话题切换 | 0% | ↓↓ | 话题跟随能力完全丧失，含上下文幻觉 |
| F-危机识别 | 0% | 新增 | 无危机响应，F3 为危险回复 |
...
| 维度 | 评分方法 | 说明 |
|------|---------|------|
| 分场景功能测试 | 6 组 30 题通过/不通过 | **诊断价值最高**——直接评估行为正确性 |
| 规则评分 | 关键词匹配（满分 5） | 底线指标——对模板化回复敏感 |
| LLM 语义评分 | 豆包大模型 API（满分 5） | 语义质量评估 |
| 自动指标 | BLEU-1 + ROUGE-L | 区分度低（0.005-0.109），仅作参考 |

### 核心发现

- 🔄 **能力跷跷板**：拒绝处理(100%) 与直接索要(0%) 不可兼得——0.8B+LoRA 的容量限制
- 📋 **模板循环**：模型仅用 4-5 个固定模板驱动所有对话，形成"伪多轮对话"
- ⚠️ **安全风险**：危机场景中模型回复"我有时候也会这样"——身份混淆+症状正常化

> 完整分析见 `论文/学术论文.md` 第 5-6 章
...
├── test/                               # 功能测试
│   ├── 测试方案/                        # 6 组 30 题测试截图
│   └── ocr_results.txt                 # OCR 提取结果
├── 论文/                               # 期末设计论文
│   ├── 学术论文.md                      # Markdown 源文件
│   ├── 学术论文.docx                   # 格式化 Word 文档
│   ├── 期末设计报告.md                  # 考核报告
│   ├── 期末考试格式要求.txt             # 格式规范
│   └── 流程记录.md                      # 写作流程记录
...
├── 实验报告.md                         # 正式考核报告
├── 同学说明文档_数据融合与泛化问题.md   # 数据融合指南
├── 白板讲义_数据处理流程.md            # 数据处理板书
├── README.md                           # 本文档
